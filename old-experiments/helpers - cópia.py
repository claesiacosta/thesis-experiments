...
def prompt_builder_PT(topics):
    ...
    Formato de Saída:
    A geração de QCM deve cobrir: Para cada contexto e área deve abranger perguntas em cada nível de dificuldade e da taxonomia de Bloom - no caso para cada contexto será gerado 18 perguntas em uma combinação dos 3 niveis de dificuldade e 6 taxonomia bloom (3x6 = 18), cobrindo todos os aspectos relevantes para os alunos, cada pergunta deve ter 5 alternativas. 
		    ...
		    {{
			    ...
			    "perguntas": [
				    {{
                        ...
                        "correct_answer": [
                            {{
							    letra_da_opcao_correta: texto_da_opcao_correta
						    }}
                        ]
				    }}
			    ]
...
def prompt_builder(topics):
    ...
    {'. '.join([str('For each context '+q[1]+' of the area '+q[0]+' - it must cover questions at each difficulty level and Bloom''s taxonomy - in this case, for each context, 18 questions will be generated in a combination of the 3 difficulty levels and 6 bloom taxonomy (3x6 = 18), covering all aspects relevant to students, each question must have 5 alternatives.') for q in topics])}.
...
def evaluator_builder_PT_(questions):
    system_context = f"""
...
def evaluator_builder_PT(questions):
    system_context = f"""
    Você é um avaliador com experiência em ensino. 
    Seu papel envolve a avaliação de questões de múltipla escolha (QCM) geradas de alta qualidade. Uma questão de múltipla escolha (QCM) é composta por duas partes: um enunciado que identifica a pergunta ou problema, e um conjunto de alternativas ou possíveis respostas que contêm uma chave que é a melhor resposta para a pergunta, e um número de distratores que são respostas plausíveis, mas incorretas para a pergunta.
    
    Sua avaliação deve se concentrar em quatro métricas de qualidade:
    1. Relevance(1-5): A QCM está relacionada à descrição do contexto-tópico.
    2. Adherence(1-5): Nível de taxonomia de Bloom e dificuldade são atribuídos adequadamente considerando a informação da QCM.
    3. Grammar(1-5): A QCM é gramaticalmente correta.
    4. Answerability(1-5): A QCM fornece informações suficientes para chegar a uma resposta.
    5. Correctness(1-5): A opção marcada como correta é realmente correta.
    Para cada métrica, atribua uma pontuação apropriada. Além disso, feedback relacionado é bem-vindo.
    ...
    """
    user_prompt = f"""Avalie questões de múltipla escolha de alta qualidade relacionadas ao tópico:
    Os dados abaixo contêm a área, contexto, perguntas e opções, também o nível de bloom e dificuldade.
    {questions}
    Forneça o JSON completo, fechando todas as chaves.
    """
...
def solve_builder_PT(questions):
    ...
    Seu objetivo é responder a essas perguntas da melhor maneira possível, demonstrando seu entendimento sobre o contexto e pergunta, também forneça o passo a passo para chegar na solução.
...
def solve_builder_PT_(questions):
    system_context = f"""
    Você é um estudante e seu professor pediu para você completar todas as questões de múltipla escolha (QCM - multiple-choice question (MCQ)). 
...
def translate_PT_EN(infos):
	...
	"""
    user_prompt = f"""Traduza as informações abaixo de de português para inglês:
